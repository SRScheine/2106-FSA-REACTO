...
https://gist.github.com/jeanettable/764eac22ec3cf3f70df6b65d048f012c
...
Instead of summing the volume vertically we will think about how much volume exists in a horizontal plane at each incremental height. The solution below goes about this by starting at the highest 'peak' or highest level, and summing the total amount of volume at that level. Then we decrement the height by one and repeat the process until a height of 0 (aka ground level) is reached. This produces an O(n*a) solution (n is the size of the array, a is the maximum number in the array). See individual comments to better understand the solution.

the 'totalVol' function will find the 'peak'
of the collection array then sum the volume
at each subsequent level until the 'ground' (at 0)
is reached.
...
The following is the main logic contributing to the calculation for this problem.
...
We could refactor the same approach to a more succinct recursive solution like so:
...
...it is possible to further simplify the above solution. We can merge the last two loops together—while keeping track of the leftwards maximum for any given block, we can also be calculating the water above it. There's no need to do these two things in separate loops:
...
Extra Resources :)
Slides
Similar Problem - AlgoExpert Link
Kadane's Algo - AlgoExpert Link
