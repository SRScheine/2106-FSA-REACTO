...
https://gist.github.com/sjmarhone/1223ec5cd67ed5ffa2b553ea152bc717

River Sizes
Learning Objective
Implement BFS for graphs, understand adjacency matrix implementation of graphs
...
Prompt the interviewee to think about how they might count all the adjacent 1s to a given node. How can they repeat this process to count all the 1s in a single river? This can be done recursively or using a stack.
...
Note: The recursive solution shown mutates the input array and the iterative approach shown does not. This is to demonstrate different approaches to the problem. An interviewee's solution can be any combination of impure/pure and recursive/iterative.

Solutions
Solution 1: Iterative
Strategy: Create an auxiliary data structure to keep track of which nodes in the matrix have already been visited. Then traverse through each element in the matrix. If the node has been visited before, move on. Otherwise visit the node and if it is a river, add its unvisited neighbors to a stack. Proceed through the stack iteratively and update the auxiliary matrix as each node is visited.
...
Strategy: Traverse through each element in the matrix. If a node is land (0), skip it. If a node is part of a river (1) then recurse through that nodes neighbors to determine the river's size. An auxiliary structure is not used to keep track of visited nodes for this solution. Instead, the input matrix itself is mutated. A 0 now indicates either a land element or a node that has been previously visited. A 1 indicates an unvisited river node.

Time Complexity: O(wh) where w is width and and h is height of the matrix. This is due to the same reasons cited in Solution 1.

Space Complexity: O(wh) where w is width and and h is height of the matrix. This is due to the callstack. Consider a worst case situation in which every node is a part of the same river. The callstack would have a recursive function call for every node before resolving.
