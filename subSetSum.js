...
https://gist.github.com/InnovaTori/6662964459d99ea5c23ce2754c51d0af

Subset Sum
Learning Objective
Apply bottoms-up problem solving (dynamic programming), compare recursive and iterative appraoches to dynamic programming.
Prompt
...
// My solution (no idea how slow it is)
function subsetSum(target, array) {
  const momo = {};
  for (let i = 0; i < array.length; i++) {
    let currNum = array[i];
    for (let key in momo) {
      let newKey = Number(key) + currNum;
      momo[newKey] = true;
    }
    momo[currNum] = true;
    // for (let keyCheck in momo) {
    //   if (keyCheck === target.toString()) {
    //     return true;
    //   }
    // }
    ...
    // console.log("WHAT IS MOMO? ", momo)
    // if (Object.keys(momo).includes(target.toString())) {
    //   return true;
    // }
  }
  return false;
...
console.log(subsetSum(2, [1, 10, 5, 3]));
console.log(subsetSum(10, [1, 10, 5, 3]));
console.log(subsetSum(9, [1, 10, 5, 3]));
console.log(subsetSum(19, [1, 10, 5, 3]));
console.log(subsetSum(7, [1, 10, 5, 3]));
